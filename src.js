...
app.innerHTML=`<section id="login" class="card"><h2>🔐 Acceso de administrador</h2><input id="loginUsername" type="text" placeholder="Usuario"><div style="display:flex;gap:6px"><input id="loginPassword" type="password" placeholder="Contraseña" style="flex:1"><button id="togglePassword" type="button">👁️</button></div><button id="loginBtn">🔓 Ingresar</button><div id="loginResult"></div></section><div id="adminApp" style="display:none"><header><h1>RAVE & ROLL 2.0</h1><p>11 de octubre de 2026 · 500 entradas</p></header>
...
<main>
<section id="dashboard" class="tab"><div id="stats" class="grid"></div><div class="card"><b>Preventa 1:</b> 125 × S/10 &nbsp; <b>Preventa 2:</b> 175 × S/15 &nbsp; <b>General:</b> 200 × S/25</div></section>
...
try{
  ...
  try{
    const c=scanner.getRunningTrackCameraCapabilities();
    const z=c.zoomFeature();

    if(z.isSupported()){
      const target=Math.min(2,z.max());
